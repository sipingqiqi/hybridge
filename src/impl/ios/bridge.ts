...
export default class IOSBridge implements Bridge {
  openSearch(type: string, hint: string, message: string): Promise<SearchResult> {
    return call<SearchResult>('openSearch', 
      JSON.stringify({ type, hint, message }), 'search', false);
  }
  ...
  toggleMenu(position: MenuPosition, show: boolean): void {
    call<any>('showActionBarPanel', JSON.stringify({ type: position, show}));
  }
  ...
  viewPdf(url: string, title: string = '', buttonTitle: string = ''): Promise<any> {
  ...
  takeUserImageMultiple(count: number = 1): Promise<any> {
    return call<any>('takeUserImageMultiple', count, 'images', false);
  }

  callCameraMultiple(count: number = 1): Promise<any> {
    return call<any>('takeUserImageMultiple', count, 'images', false);
  }
