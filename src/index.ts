...
const methods = [
  'openSearch', 'toggleSearch', 'SetH5Header', 'leftMenu', 'toggleMenu', 'rightMenu', 'onReady', 'onDataResult', 'notifyCommandFromNative'
]
 ...
 */
const openSearch = function(type: string, hint: string, message: string): Promise<SearchResult> {
   ...
   * @param btnTxt - 按钮上的文字
   */
  const viewPdf = function(url: string, title: string, btnTxt: string): Promise<any> {
   ...
   */
  const startAudioRec = function(isShow: boolean): Promise<any> {
   ...
   * 拍照
   */
  const callCamera = function(): Promise<any> {
   ...
   * @param height 
   */
  const tailorCamera = function(isCut: boolean, width: number, height: number): Promise<any> {
   ...
   * 打开地址选择窗口
   */
  const callAddress = function(): Promise<any> {
   ...
   * 打开身份证件扫描窗口
   */
  const idCardScan = function(): Promise<any> {
   ...
   * 打开银行卡扫描窗口
   */
  const getBank = function(): Promise<any> {
   ...
   * @param keyWord 
   */
  const caSign = function(name: string, type: number, keyword: string): Promise<any> {
   ...
   * 选择并导入职业信息
   */
  const getJob = function(): Promise<any> {
   ...
   * 选择并导入客户信息
   */
  const getCustomer = function(): Promise<any> {
   ...
   * @param count - 照片数量
   */
  const takeUserImageMultiple = function(count: number): Promise<any> {
   ...
   * @param count - 照片数量
   */
  const callCameraMultiple = function(count: number): Promise<any> {
   ...
   */
  const showShare = function(type: ShareType, url: string, imageUrl: string, title: string, desc: string, callback: string): Promise<any> {
