...
var IOSBridge = function () {
    ...
    _createClass(IOSBridge, [{
        ...
        value: function openSearch(type, hint, message) {
            return call('openSearch', JSON.stringify({ type: type, hint: hint, message: message }), 'search', false);
        }
    }, {
        key: 'toggleSearch',
        ...
        value: function toggleMenu(position, show) {
            call('showActionBarPanel', JSON.stringify({ type: position, show: show }));
        }
    }, {
        key: 'rightMenu',
        ...
        value: function onDataResult(eventType, eventData) {
            ...
            if (name) {
                var item = _data.data.status[name];
                if (item) {
                    item.status = true;
